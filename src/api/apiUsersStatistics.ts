...
class ApiUsersStatistics{
    url: string;
    constructor() {
        this.url = 'https://learnwords124.herokuapp.com/users/';
    }

    getUserStatistics(token, userId){
        const Url= `${this.url}${userId}/statistics`;
        const UserStatistics: Promise< Statistic > = fetch(Url, {
            method: 'GET',
            headers: {
    ...
    updateUserStatistics(token, userId, learnedWords, optional?){
        const Url= `${this.url}${userId}/statistics`;
        const Json = {
